...
def parseAgruments():
    ...
    parser.add_argument('-w', '--workspace', type=str, nargs='?', \
        help='Absolute path to the workspace folder', \
            required=True)
    ...
    parser.add_argument('-p', '--pdbFolder', type=str, nargs='?', \
        help='Folder contains pdb files of the considered protein, path start from the workspace', \
        required=True)
    parser.add_argument('-f', '--data', type=str, nargs='?', \
        help='Path to data folder, from the current directory', required=True)
...
def genProteinFolder(workspace, pdbfolder, outfolder, datafolder, meta, ions, acpype, docker, debug=False):
    if not os.path.isdir(os.path.join(workspace, pdbfolder)):
        print("Cannot find pbd folder at {}".format(os.path.join(workspace, pdbfolder)))
        exit()
...
def runAcpype(workspace, sharepath, acpype, debug=False): 
    ...
    comchmod = ['docker', 'run', '--env', 'NVIDIA_DISABLE_REQUIRE=1', \
    ...
    comrun =  ['docker', 'run', '--env', 'NVIDIA_DISABLE_REQUIRE=1', \
        '--rm', '--user', '$(id -u):$(id -g)', '--mount', \
        ...
        try:
            ...
            if debug:
                print("Looking for ions file at {}".format(\
                    os.path.join(args['workspace'],'scripts',args['data'], 'ions_'+ff+'.json')))
            ionsdict = json.load(open(os.path.join(\
                args['workspace'], 'scripts',args['data'], 'ions_'+ff+'.json')))
            print(ionsdict)
        except:
