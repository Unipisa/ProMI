...
# fasta: fasta file of wildtype protein 
# mutfile: path to the mutation file 
# outpath: path to folder to save generated fasta files of mutant proteins 
def genMutation(fasta, mutfile, outpath, debug=False):
    if debug:
        print("GENERATE MUTATIONS")
    seqs = readFasta(fasta, debug) 
    if len(seqs) <= 0:
        print("Length of fasta sequence <= 0")
        return
    muts = parseMutationFile(mutfile, seqs, outpath, debug)
    genRangeMutations(seqs, muts, outpath, debug)
...
def alignStructure(rootpath, pdbfile, docker, ligname=None, debug=False):
    if debug:
    ...
    # copy also calRMSD.py to rootpath because of dependencies 
    shutil.copy('pythonScript/calRMSD.py', rootpath)
    if debug:
        print("Finish copy pythonScript/calRMSD.py to {}".format(rootpath))

    # copy also genShareFolder.py to rootpath because of dependencies 
    shutil.copy('pythonScript/genShareFolder.py', rootpath)
    if debug:
        print("Finish copy pythonScript/genShareFolder.py to {}".format(rootpath))
...
def runMDBatch(rootpath, image, time = 1, debug=False):
    if debug:
    ...
    content += 'python3 MDBatch.py -f md -g gmx_mpi -a /tmp/amber20/miniconda/bin/gmx_MMPBSA -m 3 -n '
    content += str(time)
    if debug:
        content += ' -d \n'
    else:
...
# rootpath: abs path to workspace folder 
# image: name of the docker image used to run default = mpl
def checkProtonation(rootpath, image, debug = False):
    if debug:
        print("CHECK PROTONATION STATE")
    # inside rootpath, there is a folder named "alres", 
    # with subfolders contain alphafold prediction of each mutant protein, 
    # there is also a folder named "md"
    # which contains subfolders with aligned structures of each mutant protein 
    # run checkprotonation with each '.pdb' file inside each subfolder of 'md' folder 
    mdpath = os.path.join(rootpath, 'md')
    # print(mdpath)
    # now iterate through subfolders inside 'md' folder 
    # each subfolder is of a mutant protein 
    contents = os.scandir(mdpath)
    for content in contents: 
        # print(content)
        if os.path.isdir(content):
            alignname = os.path.join(mdpath, content, content.name + '_aligned.pdb')

            if os.path.isfile(alignname):
                if debug:
                    print('Checking PROTONATION state of file{}'.format(alignname)) 
                
                # need to run a docker container so need to generate an .sh file to run
                # python3 pythonScript/protonation.py -p [path to pdb] -o [path to output folder] -d [debug]
                # the content folder will be binded to docker container under the path "/tmp/workspace/"
                # all the files inside the docker container should have relative path to "/tmp/workspace/"
                dockeralignname = os.path.join("/tmp/workspace", content.name + '_aligned.pdb')
                script = '#!/bin/bash\n'
                script += 'WS=/tmp/workspace\n'
                script += 'cd $WS\n'
                script += 'python3 protonation.py -p ' + dockeralignname + ' -o ' + "/tmp/workspace"
                
                if debug:
                    script += ' -d'

                # write the prot.sh file save in the subfolder of each mutant protein 
                runfile = open(os.path.join(content, 'prot.sh'), 'w')
                runfile.write(script)
                runfile.close()

                #copy MDBatch.py to rootpath to run 
                shutil.copy('pythonScript/protonation.py', content)
                shutil.copy('pythonScript/genShareFolder.py', content)

                if debug:
                    print('Copy genShareFolder.py and protonation.py to {}'.format(os.path.abspath(content)))

                # now run the .sh file within a docker container 
                # now mount rootpath to docker image
                rootabs = os.path.abspath(content) 
                bind = 'src=' + rootabs + ',target=/tmp/workspace,type=bind'
                comchmod = ['docker', 'run', '--gpus', 'all', '--rm', '--user', '$(id -u):$(id -g)', \
                    '--mount', bind, image, 'chmod', '+x', '/tmp/workspace/prot.sh']
                if debug:
                    print(' '.join(comchmod))
                os.system(' '.join(comchmod))

                comrun = ['docker', 'run', '--gpus', 'all', '--rm', '--user', '$(id -u):$(id -g)', \
                    '--mount', bind, image, '/tmp/workspace/prot.sh']
                if debug:
                    print(' '.join(comrun))
                os.system(' '.join(comrun))

                if os.path.isfile(os.path.join(mdpath, content, "prot_" + content.name + '_aligned.pdb')):
                    print("Successfully generate protonated file {}".format\
                          (os.path.join(mdpath, content, "prot_" + content.name + '_aligned.pdb')))
                    print("Deleting old file at {}".format(alignname))
                    os.remove(alignname)

                if debug:
                    print("Done checking protonation state, deleting files...")
                
                os.remove(os.path.join(content, 'protonation.py'))
                if debug:
                    print("Removed file {}".format(os.path.join(content, 'protonation.py')))

                os.remove(os.path.join(content, 'genShareFolder.py'))
                if debug:
                    print("Removed file {}".format(os.path.join(content, 'genShareFolder.py')))

                os.remove(os.path.join(content, 'prot.sh'))
                if debug:
                    print("Removed file {}".format(os.path.join(content, 'prot.sh')))
...
def main():
    ...
    if len(args) < 10:
        ...
        alignStructure(rootpath, args['pdb'], args['docker'], metadic['lig'], debug)
        # add checking protonation state here 
        checkProtonation(rootpath, args['docker'], debug)

        # loop runMDBatch 5 times for 5 simulations 
        #check if number of simulations is appropriate 
        assert(args['number'] > 0), "Number of simulations need to be > 0"
        assert(args['number'] <= 10), "Number of simulations shouldn't be > 10, waste of time"

        for i in range(args['number']):
            if debug:
                print("Runing {}-th simulation".format(i+1)) 
